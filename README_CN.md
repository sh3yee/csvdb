...
这个库来自于项目中的一些真实需求，比如对 ansible 处理一些配置文件时：

- 经常需要对 CSV 做细颗粒度修改，比如改具体某一行。

具体来说：

- 只想改一列或几行，却总要手写一堆读写逻辑。
- 业务上是增删改查，但代码里只有底层 CSV 操作，写起来很散。
- 一次性脚本越写越多，后面很难复用，也不容易统一行为。
- 过滤、排序、分页、聚合这些需求会反复出现，每次从零拼很麻烦。

所以我希望这个库可以提供一套更顺手的方式：

- 用接近数据库的思路操作 CSV。
- 把高频动作（列操作、行操作、查询与聚合）做成可复用 API。
- 让日常数据处理脚本更短、更稳、更容易维护。
